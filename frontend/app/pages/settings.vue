        ...
        <div v-if="tab === 'ai'" class="settings-scroll">
          ...
          <section class="card quick-card">
            <div class="quick-card-head">
              <div class="setup-title">聚合服务快速接入</div>
              <span class="tag tag-accent">推荐</span>
            </div>
            <p class="setup-desc">
              输入聚合服务 API Key，一次写入文本、图片、视频推荐配置。
              <a class="provider-site-link" href="https://ai.centos.hk/pricing" target="_blank" rel="noopener noreferrer">
                前往 ai.centos.hk 查看模型与定价
                <ExternalLink :size="12" :stroke-width="1.8" />
              </a>
            </p>
            <div class="provider-quick-row">
              <input v-model="providerApiKey" class="input" type="password" placeholder="聚合服务 API Key" />
              <button class="btn btn-primary" :disabled="providerSaving" @click="applyProviderQuickConfig">
                <Loader2 v-if="providerSaving" :size="13" class="animate-spin" />
                <Sparkles v-else :size="13" />
                写入推荐配置
              </button>
            </div>
            <div class="provider-quick-models">
              <div v-for="q in providerQuickConfigs" :key="q.name" class="quick-model-row">
                <span class="quick-model-label">{{ serviceMeta[q.service_type].label }}</span>
                <span class="quick-model-provider">{{ q.provider }}</span>
                <span class="quick-model-list mono">
                  <span v-for="(m, i) in q.model" :key="m" :class="['quick-model', { 'is-default': i === 0 }]">
                    {{ m }}<em v-if="i === 0">默认</em>
                  </span>
                </span>
              </div>
              ...
              <div v-for="c in byType(st.type)" :key="c.id" class="config-row">
                ...
                <div class="config-main">
                  <div class="config-line">
                    ...
                    <span :class="['tag', c.api_key || isApiKeyOptional(c.provider) ? 'tag-success' : 'tag-error']">{{ c.api_key ? '已配置' : isApiKeyOptional(c.provider) ? '无需密钥' : '无密钥' }}</span>
    ...
    <div v-if="cfgDialog" class="overlay" @click.self="cfgDialog = false">
      <form class="dialog config-dialog" @submit.prevent="saveCfg">
        ...
        <div class="dialog-body config-dialog-body">
          ...
          <label class="field">
            <span class="field-label">API Key <span v-if="isApiKeyOptional(cfgForm.provider)" class="dim">(本地 ComfyUI 可留空)</span></span>
...
const providers = ['gemini', 'openai', 'volcengine', 'minimax', 'comfyui', 'jimeng']
const providerSelectOptions = computed(() => providers.map(p => ({ label: p, value: p })))
const serviceMeta = {
  text: { label: '文本', desc: '剧本改写、角色场景提取、分镜拆解等 Agent 文本能力' },
  image: { label: '图片', desc: '角色图、场景图与镜头图等静态图像生成' },
  video: { label: '视频', desc: '镜头视频直出生成，支持云端服务与本地 ComfyUI' },
}
...
const providerQuickConfigs = [
  { service_type: 'text', provider: 'gemini', name: '聚合文本服务 · Gemini', base_url: 'https://ai.centos.hk', model: ['gemini-3.1-pro-preview', 'gemini-3.5-flash', 'gemini-3-flash-preview'], priority: 100 },
...
function isApiKeyOptional(provider) { return provider === 'comfyui' }
...
async function applyProviderQuickConfig() {
  const apiKey = providerApiKey.value.trim()
  if (!apiKey) { toast.warning('请填写聚合服务 API Key'); return }
  providerSaving.value = true
  try {
    for (const preset of providerQuickConfigs) {
...
.page-title {
  font-size: 32px; font-weight: 800; letter-spacing: 0;
...
.settings-title { font-size: 22px; font-weight: 800; letter-spacing: 0; }
...
.provider-site-link:hover { text-decoration: underline; }
.provider-quick-row {
  display: grid;
  grid-template-columns: minmax(0, 1fr) auto;
  gap: 10px;
}
.provider-quick-models {
  margin-top: 14px;
  display: flex;
...
.quick-model-provider {
  ...
  padding: 1px 6px;
  border-radius: 4px;
  background: var(--accent-bg, rgba(20,125,104,0.10));
  color: var(--accent, #147d68);
  font-size: 9px;
  font-weight: 600;
...
.quick-model.is-default { color: var(--text-1); font-weight: 600; }
.quick-model em {
  font-style: normal;
  margin-left: 4px;
  padding: 0 5px;
